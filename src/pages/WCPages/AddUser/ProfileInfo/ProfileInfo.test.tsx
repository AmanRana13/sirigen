import {commonStateData} from '_mocks_/commonMocks';
import {fireEvent, render, screen} from '../../../utilities/test-utils';
import {ProfileInfoComponent} from './ProfileInfo.component';
import ProfileInfo from './ ProfileInfo.container';
import {store} from '../../../store/store';
import {
  hideApplicationLoader,
  showApplicationLoader,
} from 'common/ApplicationLoader';
import {
  createAccount,
  fetchSeniorInfo,
  saveProfileInfo,
  uplaodImage,
} from './ProfileInfo.action';
import {showToast} from 'common/Toast';
import {setSeniorDetail} from 'store/commonReducer/common.action';
import {GetSeniorData} from 'globals/enums';
const data = {
  senior_info: {
    name: {
      first_name: 'Kaptan',
      last_name: 'SleepMat',
      middle_name: 'Singh',
    },
    mobile_number: '9999999999',
    gender: 'Male',
    dob: '1979-06-11T18:30:00+00:00',
    user_id: 'senior-042ec8bb092f4442b65fb8705f82f324',
    account_id: '1fc17069353f4da7b15f632748b429ed',
    created_date: '2021-08-04T06:53:21.268517+00:00',
    email: 'kaptan.singh@clearcaptions.com',
    home_phone: '9999999999',
    emergency_phone: '8888888888',
    social_media_links: 'https://wwww.insta.com',
  },
  persona: [],
};
const isProfileCreated = true;
const accountInfo = {
  account_id: '7a97969b5658469b807c8b2797ec62ee',
  senior_id: 'senior-11cca3e1a63e4c7589e9b18ea96f37b3',
};
beforeAll(() =>
  localStorage.setItem('accountInfo', JSON.stringify(accountInfo)),
);

describe('ProfileInfo container', () => {
  test('should render ProfileInfo container', () => {
    render(
      <ProfileInfo
        createAccount={jest.fn()}
        fetchSeniorInfo={jest.fn().mockImplementation(() => Promise.resolve())}
        saveProfileInfo={jest.fn()}
        uplaodImage={jest.fn()}
        getImage={jest.fn()}
        setProfileImage={jest.fn()}
        removeImage={jest.fn()}
      />,
    );
    const element = screen.queryByText('Profile Info');
    expect(element).toBeInTheDocument();
    const saveButton = screen.getByRole('button', {name: /save/i});
    expect(saveButton).toBeDisabled();
  });
});

describe('ProfileInfo component', () => {
  test('should render ProfileInfo component', () => {
    render(
      <ProfileInfoComponent
        createAccount={jest.fn()}
        fetchSeniorInfo={jest
          .fn()
          .mockImplementation(() =>
            Promise.resolve(commonStateData.seniorDetail),
          )}
        saveProfileInfo={jest.fn().mockImplementation(() => Promise.resolve())}
        uplaodImage={jest.fn()}
        getImage={jest.fn().mockImplementation(() => Promise.resolve())}
        setProfileImage={jest.fn()}
        removeImage={jest.fn()}
        isOnboardingInfo={true}
      />,
      {initialState: commonStateData},
    );
    const element = screen.queryByText('Profile Info');

    expect(element).toBeInTheDocument();
  });
  test('should render save button in ProfileInfo component', async () => {
    const saveProfileInfo = jest
      .fn()
      .mockImplementation(() => Promise.resolve());
    render(
      <ProfileInfoComponent
        createAccount={jest.fn()}
        fetchSeniorInfo={jest
          .fn()
          .mockImplementation(() =>
            Promise.resolve(commonStateData.seniorDetail),
          )}
        saveProfileInfo={saveProfileInfo}
        uplaodImage={jest.fn()}
        getImage={jest.fn().mockImplementation(() => Promise.resolve())}
        setProfileImage={jest.fn()}
        removeImage={jest.fn()}
      />,
      {initialState: commonStateData},
    );
    const element = screen.getByRole('button', {name: /Save/i});
    fireEvent.click(element);
    await store.dispatch(showToast('Saved Successfully', 'success'));
    expect(store.getState().toast).toStrictEqual({
      duration: 4000,
      message: 'Saved Successfully',
      open: true,
      type: 'success',
    });
  });
});
describe('profileInfo action creaters', () => {
  test('dispatch createAccount action', async () => {
    await store.dispatch(showApplicationLoader());
    await store.dispatch(createAccount(data, isProfileCreated));
    await store.dispatch(hideApplicationLoader());
    await store.dispatch(
      showToast(
        // eslint-disable-next-line max-len
        'Account saved successfully!',
        'success',
      ),
    );
    await store.dispatch(setSeniorDetail(GetSeniorData.SENIOR));
    expect(store.getState().applicationLoader).toBeTruthy();
    expect(store.getState().toast).toStrictEqual({
      duration: 4000,
      message: 'Account saved successfully!',
      open: true,
      type: 'success',
    });
  });

  test('fetchSeniorInfo', async () => {
    await store.dispatch(showApplicationLoader());
    await store.dispatch(fetchSeniorInfo(data));
    global.fetch = jest.fn(() =>
      Promise.resolve({json: () => ({status: 'ok', data: 'data'})}),
    );
    await store.dispatch(hideApplicationLoader());
    expect(store.getState().applicationLoader).toBeTruthy();
  });
  beforeAll(() =>
    localStorage.setItem('accountInfo', JSON.stringify(accountInfo)),
  );
  test('dispatch uplaodImage action', async () => {
    await store.dispatch(showApplicationLoader());
    await store.dispatch(
      uplaodImage({
        format: 'jpeg',
        image:
          // eslint-disable-next-line max-len
          '',
      }),
    );
    await store.dispatch(hideApplicationLoader());
    expect(store.getState().applicationLoader).toBeTruthy();
  });
  test('dispatch saveProfileInfo action', async () => {
    await store.dispatch(showApplicationLoader());
    await store.dispatch(saveProfileInfo(data, {}));
    await store.dispatch(hideApplicationLoader());
    await store.dispatch(showToast('Saved Successfully', 'success'));
    expect(store.getState().toast).toStrictEqual({
      duration: 4000,
      message: 'Saved Successfully',
      open: true,
      type: 'success',
    });
    expect(store.getState().applicationLoader).toBeTruthy();
  });
});
