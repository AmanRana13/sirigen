...
import {
  toggleIncompleteTag,
  getLawtonBrodyAssessment,
  postLawtonBrodyAssessment,
  toggleLawtonBrodyAssessmentViewState,
  getLawtonBrodyAssessmentHistoryFail,
  getLawtonBrodyAssessmentHistorySuccess,
  getLawtonBrodyAssessmentHistory,
  updateLawtonBrodyAssessmentHistoryPageNumber,
} from './LawtonBrodyAssessment.action';
import {assessmentStyle} from '../../AssessmentTable.style';
import {mapLawtonBrodyFormData} from 'services/assessmentService/ADLAssessmentService/lawtonBrodyService/LawtonBrody.services';
import LawtonBrodyAssessmentTable from '../AdlAssessmentTable';
import {ScoreTable} from '../ADLScoreTabel';
import {
  ILawtonAssessmentHistoryTableProps,
...
const LawtonBrodyAssessment = () => {
  ...
  */
  ...
  const surveyCount: number = React.useMemo(() => {
    let count: number = 0;
    surveyState?.forEach((item: ILawtonBrodyADLAssessmentSurveyData) => {
  ...
  const handleSaveSubmit = (
    ...
    const assessmentData: IPostLawtonBrodyActionParams = {
      survey: [...surveyState],
      type: type,
    ...
    if (type === AssessmentStatus.Submit) {
      if (!checkIfFormFilled()) {
        ...
        dispatch(postLawtonBrodyAssessment(assessmentData));
      }
    } else {
      // save the form only if assessment is updated
        ...
        if (assessmentStatus === AssessmentStatus.Save) {
          ...
          dispatch(
            openOverlayDialog({
              ...
              firstMessage:
                'Lawton-Brody assessment has been reset successfully ',
            }),
          );
   ...
   */
  const handleClose = () => {
    dispatch(toggleLawtonBrodyAssessmentViewState(false, ''));
    dispatch(getLawtonBrodyAssessment());
  };
  ...
  const metaData = React.useMemo(() => {
    const lawtonMetaData = new AssessmentMetaDataModel(
      constructName(
  ...
  return (
    <AssessmentWrapper
      ...
      heading='Lawton-Brody ADL'
      infoText='Scoring: For each category, click the item that most closely
      resembles the client’s highest functional level (either 0 or 1).
      NOTE: the Scores total at the bottom of this form.'>
      <>
        <Box data-testid='LawtonBrodyAssessmentComponent' mt={2} ref={ref}>
          {surveys &&
            surveyState.map(
              (item: ILawtonBrodyADLAssessmentSurveyData, index: number) => {
                const labelPrefix = String.fromCharCode(index + 65);
                return (
                  <LawtonBrodyAssessmentTable
                    key={item.title}
                    label={item.title}
          ...
          <Box display='flex' justifyContent='left' pt={1}>
            ...
            <Typography paragraph={true} className={classes.note}>
              ...
              function, independent) for women and 0 through 5 for men to avoid
...
const LawtonAssessmentHistoryTable = React.memo(
  ({
    historyTableData,
    handleSaveSubmit,
    ...
    const actionCreatorProps: IAssementHistoryActionCreators = {
      ...
      getData: getLawtonBrodyAssessmentHistory,
      success: getLawtonBrodyAssessmentHistorySuccess,
      error: getLawtonBrodyAssessmentHistoryFail,
      pageChange: updateLawtonBrodyAssessmentHistoryPageNumber,
    };
     ...
     */
    const onClickViewHistory = async (history: any) => {
      if (isAssessmentDataUpdated) {
        ...
        dispatch({
          type: GET_ASSESSMENT,
          payload: {
            surveys: mapLawtonBrodyFormData(history.formData),
            dateTime: dateTime,
    ...
    return (
      <AssessmentHistory
        tableLabel='Previous Lawton-Brody ADL Assessments'
...
export default LawtonBrodyAssessment;
